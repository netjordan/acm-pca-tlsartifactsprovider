package org.netjordan.ACMPCATLSArtifactsProvider.test;

import com.amazonaws.services.acmpca.model.CertificateAuthority;
import com.amazonaws.services.elasticmapreduce.spi.security.TLSArtifacts;
import org.bouncycastle.operator.OperatorCreationException;
import org.junit.Test;
import org.netjordan.ACMPCATLSArtifactsProvider.ACMPCATLSArtifactsProvider;

import java.io.IOException;
import java.nio.ByteBuffer;
import java.security.KeyPair;
import java.security.cert.X509Certificate;
import java.util.List;

public class ACMPCATLSArtifactsProviderTest {
    ACMPCATLSArtifactsProvider acmpcatlsArtifactsProvider;

    public ACMPCATLSArtifactsProviderTest() {
        this.acmpcatlsArtifactsProvider = new ACMPCATLSArtifactsProvider();
    }

    @Test
    public void testCanListPCAProviders() {
        List<CertificateAuthority> certificateAuthorities = this.acmpcatlsArtifactsProvider.listCertificateAuthorities();

        assert certificateAuthorities.size() > 0;

        for (CertificateAuthority certificateAuthority : certificateAuthorities) {
            System.out.println(certificateAuthority.getArn());
        }
    }

    @Test
    public void testCanGetTLSArtifacts() {
        TLSArtifacts tls = this.acmpcatlsArtifactsProvider.getTlsArtifacts();

    }

    @Test
    public void testCanGenerateKeyPair() {
        try {
            KeyPair keyPair = this.acmpcatlsArtifactsProvider.generateKeyPair();
        }
        catch (Exception ex) {
            assert false;
        }
    }


    @Test
    public void testCanGenerateCSR() {
        try {
            KeyPair keyPair = this.acmpcatlsArtifactsProvider.generateKeyPair();
            ByteBuffer csr = this.acmpcatlsArtifactsProvider.generateCSR(keyPair);

        } catch (OperatorCreationException | IOException e) {
            e.printStackTrace();
            assert false;
        }
    }

    @Test
    public void testCanConvertToCertificate() {
        String certificateString = """
           -----BEGIN CERTIFICATE-----
           
           -----END CERTIFICATE-----
            """;

        List<X509Certificate> certificateList = null;

        try {
            certificateList = this.acmpcatlsArtifactsProvider.convertToCertificatelist(certificateString);
        }
        catch (Exception ex) {
            throw ex;
        }

        assert certificateList.size() > 0;
    }



}
